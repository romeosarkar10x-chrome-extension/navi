...
export function ChatView({ messages, draft, setDraft, onSend, model, onOpenModel, busy }: ChatViewProps) {
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col">
      <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto py-4 px-[14px] flex flex-col gap-[14px] navi-scroll">
        {messages.map((m, i) =>
          m.kind === 'action' ? (
            <AgentActionCard key={i} type={m.type} label={m.label} status={m.status} detail={m.detail} defaultOpen={m.open} />
          ) : (
            <ChatMessage key={i} role={m.role} meta={m.meta} initials="JD">
              {m.body}
            </ChatMessage>
          ),
        )}
      ...
      <div className="flex-none border-t border-line bg-surface-base pt-[10px] px-3 pb-3 flex flex-col gap-[9px]">
        <div className="flex gap-[6px] flex-wrap">
          ...
          <ContextPill icon="scan-text" onRemove={() => {}}>
            Selected text
          </ContextPill>
        </div>
        <QuickActions
          actions={[
            ...
            { icon: 'table', label: 'Extract data', onClick: () => onSend('Extract the job listings as a table') },
            { icon: 'camera', label: 'Screenshot & analyze' },
            { icon: 'text-cursor-input', label: 'Fill form' },
          ]}
        />
        <PromptInput value={draft} onChange={setDraft} onSend={() => onSend(draft)} model={model} onModelClick={onOpenModel} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col pt-4 px-4 pb-5 overflow-y-auto navi-scroll">
      ...
      <div className="flex items-center gap-[6px] text-sm text-muted mb-4">
        <Icon name="eye" size={13} /> Looking at: <b className="text-body">“Apply” button</b> on the listing card
      </div>
      ...
      <div className="flex gap-2 mt-[14px] pt-[14px] border-t border-line">
        {paused ? (
          <Button variant="primary" size="sm" icon="play" onClick={() => setPaused(false)}>
            Resume
          </Button>
        ) : (
          <Button variant="secondary" size="sm" icon="pause" onClick={() => setPaused(true)}>
            Pause
          </Button>
        )}
