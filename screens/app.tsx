...
const GREETING: ChatTurn[] = [
  {
    ...
    body: (
      <span>
        Hey! I can see this jobs page. Ask me to summarize it, pull the listings into a table, or apply to matching
        roles — I'll show each step before running.
      </span>
    ),
  },
];
...
function EmptyView({ kind, onBack }: { kind: 'history' | 'recipes'; onBack: () => void }) {
  const cfg: { icon: IconName; t: string; d: string } =
    kind === 'history'
      ? { icon: 'history', t: 'No history yet', d: 'Conversations you have with Navi will show up here, grouped by day.' }
      : { icon: 'bookmark', t: 'No recipes yet', d: 'Save an automation as a recipe to run it again on any matching page.' };
  return (
    <div className="flex-1 min-h-0 flex flex-col items-center text-center justify-center pt-4 px-4 pb-5 overflow-y-auto navi-scroll">
      ...
      <div className="mt-4">
        <Button variant="secondary" size="sm" icon="arrow-left" onClick={onBack}>
          Back to chat
        </Button>
      </div>
    </div>
...
export function App() {
      ...
      if (/apply/i.test(t)) {
        setMessages((m) => [
          ...m,
          {
            ...
            body: (
              <span>
                I found <strong>12 matching roles</strong>. I'll apply to each, pausing for review before any submit.
                Starting now.
              </span>
            ),
          },
        ...
        setMessages((m) => [
          ...
          },
          {
            ...
            body: (
              <span>
                Extracted 12 listings. Top result: <strong>Senior SWE · Acme · $180–210k</strong>. Want this as CSV?
              </span>
            ),
          },
        ]);
        ...
        setMessages((m) => [
          ...m,
          {
            ...
            body: (
              <span>
                This is a <strong>job search results</strong> page with 1,284 listings. The top roles are senior backend
                and full-stack positions in the Bay Area, most posted this week.
              </span>
            ),
          },
        ]);
  ...
  if (view === 'welcome') {
    ...
    main = <ConnectView provider={provider} onBack={() => setView('welcome')} onDone={() => setView('chat')} />;
  } else if (view === 'settings') {
    main = <SettingsView onBack={() => setView('chat')} />;
  } else if (view === 'task') {
    main = <TaskView onStop={() => setView('chat')} />;
  } else if (view === 'history' || view === 'recipes') {
    main = <EmptyView kind={view} onBack={() => setView('chat')} />;
  } else {
    main = (
      <ChatView
        messages={messages}
