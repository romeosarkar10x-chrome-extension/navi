...
export function SettingsView({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col pt-4 px-4 pb-5 overflow-y-auto navi-scroll">
      ...
      >
        <Icon name="arrow-left" size={14} /> Back to chat
      </button>
      ...
      <Section title="Provider">
        ...
        {provider === 'cloud' ? (
          <Row label="API key">
            <APIKeyInput size="sm" value="sk-ant-api03-9f2c" onChange={() => {}} />
          </Row>
        ) : (
          <Row label="Status">
      ...
      <Section title="Agent">
        <Row label="Auto-execute actions" hint="Run without asking each time">
          <Switch checked={autoExec} onChange={setAutoExec} />
        </Row>
        <Row label="Max autonomous steps" hint={`${maxSteps} before pausing`}>
          <div className="w-[120px]">
            <Slider value={maxSteps} min={1} max={50} onChange={setMaxSteps} />
          </div>
        </Row>
        <Row label="Action speed">
          <Select size="sm" options={['Slow', 'Normal', 'Fast']} value={speed} onChange={(e) => setSpeed(e.target.value)} />
        </Row>
      </Section>
      ...
      <Section title="General">
        ...
        <Row label="Sidebar position">
          <Select size="sm" options={['Right', 'Left']} value={position} onChange={(e) => setPosition(e.target.value)} />
        </Row>
      </Section>

      <Section title="Data & privacy">
        <Row label="Save conversation history">
          <Switch checked={history} onChange={setHistory} />
        </Row>
        ...
        {MODELS.map((m) => {
          ...
          return (
            ...
            >
              ...
              {on ? <Icon name="check" size={15} /> : <StatusDot tone="success" />}
            </button>
          );
        })}
      </div>
