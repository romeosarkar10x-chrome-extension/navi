    ...
    <div className="flex-1 min-h-0 flex flex-col items-center text-center pt-[34px] px-[22px] pb-[22px] overflow-y-auto navi-scroll">
      <div className="w-[88px] h-[88px] rounded-[22px] bg-surface-card border border-line flex items-center justify-center shadow-[var(--glow-soft)] mb-5">
        <svg width="56" height="56" viewBox="0 0 32 32" fill="none">
          <rect x="3.1" y="5.1" width="25.8" height="21.8" rx="5.2" stroke="var(--text-strong)" strokeWidth="2" />
          <line x1="20.6" y1="6.2" x2="20.6" y2="25.8" stroke="var(--text-strong)" strokeWidth="2" />
          <circle cx="24.75" cy="16" r="5.6" fill="#22DDD0" opacity="0.26" />
          <circle cx="24.75" cy="16" r="2.7" fill="#22DDD0" />
        </svg>
      </div>
      <h1 className="font-display text-2xl font-bold text-strong tracking-[-0.02em] leading-[1.2]">Hey, I'm Navi</h1>
      ...
      <div className="flex flex-col gap-1 w-full my-6 text-left">
        {POINTS.map((p, i) => (
          <div className="flex gap-[11px] py-[11px] px-3 rounded-md" key={i}>
      ...
      <div className="flex flex-col gap-[9px] w-full">
        <Button variant="primary" full icon="sparkles" onClick={() => onConnect('cloud')}>
          Connect to Claude
        </Button>
        <Button variant="secondary" full icon="cpu" onClick={() => onConnect('local')}>
          Use a local model
        </Button>
      </div>
    </div>
    ...
    <div className="flex-1 min-h-0 flex flex-col pt-4 px-4 pb-5 overflow-y-auto navi-scroll">
      ...
      {isCloud ? (
        <>
          ...
          <APIKeyInput value={key} onChange={(e) => setKey(e.target.value)} />
          {!ready && <div className="h-2" />}
          {!ready && (
            <Banner tone="info" title="Where do I find this?">
        ...
        <>
          ...
          <Input icon="link" value={endpoint} onChange={(e) => setEndpoint(e.target.value)} />
          <div className="flex items-center gap-3 mt-3">
            <Button variant="secondary" size="sm" icon="plug-zap" onClick={() => setTested(true)}>
              Test connection
            </Button>
            {tested && (
      ...
      <div className="mt-[22px]">
        <Button variant="primary" full disabled={!ready} iconRight="arrow-right" onClick={onDone}>
          Start using Navi
        </Button>
      </div>
    </div>
